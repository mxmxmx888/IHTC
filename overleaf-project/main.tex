...
The primary comparison metric is a common post-processed weighted IHTC solution cost calculated from the final feasible schedule of each implementation. This avoids comparing internal objective values that may be defined or scaled differently. Any weights defined by the IHTC evaluation specification are used only for this common score; no additional weights are introduced for hard-constraint feasibility reporting~\cite{ceschia2025ihtc}. Measures such as removed admissions, inactive nurse assignments, room-occupancy standard deviation, and transformation skip rate are retained as MDEO diagnostic metrics, but are not used as the main benchmark-quality measure.
