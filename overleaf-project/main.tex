...
\begin{itemize}
    ...
    \item \textbf{Admission scope and uniqueness:} transformations T1--T3 add only unscheduled, non-mandatory patients and prevent duplicate admissions for the selected patient. T4 removes optional patients only, T5 and T7 may remove any scheduled patient, and T6 replaces optional patients.
    ...
    \item \textbf{Objective-based penalties:} aggregate surgeon and theatre operating-time excess, nurse workload excess, uncovered room-shifts, uncovered workload, workload imbalance, inactive nurse assignments, admission delay, room-occupancy standard deviation, and unscheduled optional patients are minimised as objective values.
\end{itemize}
...
The experiment evaluates solutions in two stages: feasibility first, followed by solution quality and search performance. Every final schedule is independently checked against the full hard-constraint set. A run is feasible only when it has no hard-constraint violations. If no feasible candidate is produced within the time limit, the run is classified as infeasible and is excluded from feasible-solution quality summaries. Its constraint-violation values are reported as diagnostics instead.
...
\begin{itemize}
    ...
    \item Room age-group compatibility violations, reported as a hard-constraint diagnostic.
\end{itemize}
...
\begin{itemize}
    ...
    \item final feasibility status and the selected candidate's objective and diagnostic values;
...
\begin{itemize}
    ...
    \item \textbf{Pareto Selection:} the final feasible Pareto candidates were retained. Each reported candidate is selected using an explicit instance-level rule based on the reported measures. Tests~01--09 use the feasible candidate with the highest number of admitted patients. Test~10 uses an admissions-and-nursing-coverage trade-off.
...
\begin{itemize}
    \item final feasibility status and selected objective and diagnostic values;
...
Table~\ref{tab:instance-results} reports per-instance feasibility and runtime. Selection of MDEO-Cloud Pareto candidates uses documented instance-level criteria rather than a common score. Tests~01--09 use the feasible candidate with the highest number of admitted patients. For Test~10, the reported candidate balances admissions with nursing coverage: it admits 349 patients, includes 44 nurse assignments, and has the lowest uncovered nursing workload among the resource-feasible candidates. The associated feasibility, coverage, resource, and runtime measures are reported separately.
...
IHTC 2024 Test 10 & 525 & Yes & 32,676 & -- & -- \\
\bottomrule
\end{tabular}%
}
\caption{Per-instance feasibility and runtime results.}
...
No single score is calculated from the selected MDEO schedules. No mean, standard deviation, or statistical significance claim is reported because each instance was executed once.
...
Interpretation should be limited to the evidence in Tables~\ref{tab:instance-results} and~\ref{tab:feasibility-results} and the performance charts. It should describe how feasibility, admissions, nursing coverage and workload, resource use, and runtime differed across completed instances, together with the constraints or search behaviours that limited each approach. Full schedules, raw logs, Pareto fronts, and Excel exports should remain in the project repository or appendix.
...
\begin{itemize}
    ...
    \item \textbf{Pareto-solution selection:} MDEO-Cloud can return many final Pareto candidates from one execution. These candidates are correlated outputs of the same search and may represent different objective trade-offs, so they are not treated as independent samples and are not averaged. The reported candidate for each instance follows an explicit selection rule based on the reported measures. Tests~01--09 use the feasible candidate with the highest number of admitted patients, while Test~10 uses an admission--coverage trade-off.
\end{itemize}
